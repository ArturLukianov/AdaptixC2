...
type StegoHTTP struct {
	AgentConnects Map
	JobConnects   Map
	Listener      net.Listener
	Config        StegoHTTPConfig
	Name          string
	Active        bool
}
...
func (handler *StegoHTTP) Start(ts Teamserver) error {
	var err error = nil
	address := fmt.Sprintf("%s:%d", handler.Config.HostBind, handler.Config.PortBind)

	fmt.Println("  ", "Started Stego HTTP listener: "+address)
...
func (handler *StegoHTTP) handleConnection(conn net.Conn, ts Teamserver) {
	var (
		sendData []byte
		recvData []byte
		err      error
...
func (handler *StegoHTTP) Stop() error {
	var (
		err          error = nil
...
func (handler *StegoHTTP) bannerError(conn net.Conn) {
	_ = conn.Close()
}
